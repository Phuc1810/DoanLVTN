...
import adminNewsCommentApi from '../../api/adminNewsCommentApi'
...
export default function AdminNewsCommentsPage() {
  ...
  const [replyModal, setReplyModal] = useState({ isOpen: false, commentId: null, commentText: '', customerName: '', phanHoiCu: '' })
  const [confirmModal, setConfirmModal] = useState({ isOpen: false, commentId: null, isHidden: false })
  ...
  const fetchcomments = () => {
    ...
    adminNewsCommentApi.list(filters)
      .then(res => {
  ...
  const executeToggleStatus = async () => {
    ...
    try {
      const res = await adminNewsCommentApi.toggleStatus(confirmModal.commentId)
      showToast(res.message || 'Đã cập nhật trạng thái')
      fetchcomments()
      setConfirmModal({ isOpen: false, commentId: null, isHidden: false })
    } catch (err) {
      alert(err.message)
    } finally {
  ...
  const openReplyModal = (comment) => {
    setReplyContent(comment.PhanHoi || '')
    setReplyModal({
      isOpen: true,
      commentId: comment.MaBL,
      commentText: comment.NoiDung,
      customerName: comment.khach_hang?.HoTen || 'Khách hàng',
      phanHoiCu: comment.PhanHoi
    })
  }

  const closeReplyModal = () => {
    setReplyModal({ isOpen: false, commentId: null, commentText: '', customerName: '', phanHoiCu: '' })
    setReplyContent('')
  }
  ...
  const submitReply = async (e) => {
    ...
    setActionLoading(true)
    try {
      const res = await adminNewsCommentApi.reply(replyModal.commentId, { PhanHoi: replyContent.trim() })
  ...
  return (
    <>
      <div className="page-header d-flex justify-content-between align-items-center mb-4">
        <div>
          <h1 className="page-title fw-bold text-dark mb-1" style={{ fontSize: '24px' }}>Quản lý Bình luận Tin tức</h1>
          <p className="text-muted mb-0" style={{ fontSize: '14.5px' }}>Kiểm duyệt và phản hồi bình luận từ khách hàng</p>
        </div>
      </div>
          ...
          <div className="col-md-5">
            <div className="input-group" style={{ backgroundColor: '#f9fafb', borderRadius: '10px', overflow: 'hidden', border: '1px solid #f3f4f6' }}>
              ...
              <input 
                ...
                placeholder="Tìm tên khách hàng, nội dung bình luận..." 
      ...
      {!state.loading && !state.error && state.rows.length === 0 && (
        <div className="text-center py-5 bg-white rounded-4 shadow-sm">
          ...
          <h5 className="text-muted">Không tìm thấy bình luận nào</h5>
        </div>
      )}

      {!state.loading && !state.error && state.rows.length > 0 && (
        <div className="row g-4">
          {state.rows.map(comment => {
            const isHidden = comment.TrangThai === 'Đã ẩn'
            const hasReply = !!comment.PhanHoi
            
            return (
              <div key={comment.MaBL} className="col-12">
                  ...
                  <div className="card-body p-4">
                    <div className="d-flex justify-content-between align-items-start mb-3">
                      <div className="d-flex align-items-center gap-3">
                        <img 
                          src={'https://ui-avatars.com/api/?name=' + (comment.khach_hang?.HoTen || 'U')} 
                          alt="Avatar" 
                          className="rounded-circle object-fit-cover shadow-sm"
                          style={{ width: '48px', height: '48px', border: '2px solid #e2e8f0' }}
                        />
                        <div>
                          <h6 className="mb-1 fw-bold text-dark">{comment.khach_hang?.HoTen || 'Khách hàng'}</h6>
                          <div className="d-flex align-items-center gap-2">
                            <span className="text-muted" style={{ fontSize: '13px' }}>{new Date(comment.NgayBinhLuan).toLocaleDateString('vi-VN')}</span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mb-3 px-3 py-2 bg-light rounded-3 d-inline-block">
                      <Link to={`/news/${comment.MaTin}`} target="_blank" className="text-decoration-none fw-medium" style={{ color: '#0265d2', fontSize: '14px' }}>
                        Tin tức: {comment.tin_tuc?.TieuDe || 'Tin tức không tồn tại'}
                      </Link>
                    </div>
                    ...
                    {hasReply && (
                      <div className="mb-4 ms-4 p-3 rounded-4" style={{ backgroundColor: '#f1f5f9', borderLeft: '4px solid #3b82f6' }}>
                        ...
                        <p className="mb-0 text-dark" style={{ fontSize: '14.5px', lineHeight: '1.5' }}>{comment.PhanHoi}</p>
                      </div>
                    )}
                    ...
                    <div className="d-flex gap-2 pt-3 border-top">
                      <button 
                        ...
                        onClick={() => setConfirmModal({ isOpen: true, commentId: comment.MaBL, isHidden })}
                        disabled={actionLoading}
                      >
                        {isHidden ? <Eye size={16} /> : <EyeOff size={16} />}
                        {isHidden ? 'Hiển thị lại' : 'Ẩn bình luận'}
                      </button>
                      <button 
              ...
              <div className="modal-content" style={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
                <div className="modal-body text-center p-4">
                  ...
                  <h5 className="mb-4 fw-bold" style={{ color: '#1f2937', fontSize: '20px', lineHeight: '1.5' }}>
                    {confirmModal.isHidden ? 'Bạn có muốn hiển thị lại bình luận này không?' : 'Bạn có muốn ẩn bình luận này không?'}
                  </h5>
                  <div className="d-flex justify-content-center gap-3 mt-2">
                    <button 
                      type="button" 
                      className="btn fw-medium modal-btn-cancel" 
                      onClick={() => setConfirmModal({ isOpen: false, commentId: null, isHidden: false })}
                      disabled={actionLoading}
                    >
                      Hủy
                    </button>
                ...
                <form onSubmit={submitReply}>
                  ...
                  <div className="modal-body p-4">
                    <div className="p-3 mb-4 rounded-3" style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                      ...
                      <span className="text-muted fst-italic" style={{ fontSize: '14px' }}>"{replyModal.commentText}"</span>
                    </div>
